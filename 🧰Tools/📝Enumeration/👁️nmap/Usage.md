...
Scan in verbose mode (`-v`), enable OS detection, version detection, script scanning, and traceroute (`-A`), with version detection (`-sV`) against the target IP.

	nmap -v -A -sV $ip

Search for Possible Vulnerabilities in ports or services:
